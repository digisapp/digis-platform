...
import { Ticket, Plus, BarChart3, Calendar, CheckCircle2, DollarSign, Sparkles, Unlock } from 'lucide-react';
...
export default function CreatorStreamsPage() {
  ...
  const filteredShows = shows.filter(show => {
    ...
    const matchesAccess = accessFilter === 'all' ||
      (accessFilter === 'free' && show.ticketPrice === 0) ||
      (accessFilter === 'paid' && show.ticketPrice > 0);
    return matchesStatus && matchesCategory && matchesAccess;
  });
  ...
  const stats = {
    ...
    freeStreams: shows.filter(s => s.ticketPrice === 0).length,
    paidStreams: shows.filter(s => s.ticketPrice > 0).length,
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 md:pl-20">
      <div className="container mx-auto px-4 pt-0 md:pt-10 pb-24 md:pb-8">
        ...
        {/* Access Type + Category Filter Pills */}
                ...
                className={`px-4 py-2 rounded-full font-semibold text-xs transition-all duration-300 flex items-center gap-1 ${
                  accessFilter === access
                    ? access === 'free'
                      ...
                      : 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-[0_0_15px_rgba(34,211,238,0.4)]'
                    : 'backdrop-blur-xl bg-white/10 text-gray-300 border border-white/20 hover:border-cyan-500/50'
                }`}
              >
                {access === 'free' && <Unlock className="w-3 h-3" />}
                {access === 'paid' && <Ticket className="w-3 h-3" />}
          ...
          <div className="relative overflow-hidden rounded-3xl p-12 text-center bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl">
            ...
            <div className="relative">
              ...
              <h3 className="text-2xl font-black text-white mb-3">
                No {accessFilter !== 'all' ? `${accessFilter} ` : ''}{statusFilter === 'scheduled' ? 'upcoming' : 'completed'} streams
              </h3>
