...
import { hubItems, hubPacks, hubPackItems, hubPurchases, users } from '@/db/schema';
...
export async function GET(
  ...
  try {
    ...
    const conditions = [
      eq(hubItems.creatorId, creator.id),
      eq(hubItems.status, 'live'),
    ];
    ...
    const [items, totalResult, packs] = await Promise.all([
      db.select()
        ...
        .orderBy(desc(hubItems.publishedAt))
        .limit(limit + 1)
        .offset(offset),
      ...
      offset === 0
        ? db.query.hubPacks.findMany({
            where: and(eq(hubPacks.creatorId, creator.id), eq(hubPacks.status, 'live')),
            orderBy: [desc(hubPacks.createdAt)],
          })
        : Promise.resolve([]),
    ]);
    ...
    if (currentUser && (itemsToReturn.length > 0 || packs.length > 0)) {
      const purchases = await db.select()
        .from(hubPurchases)
        .where(eq(hubPurchases.buyerId, currentUser.id));
    ...
    console.error('[PROFILE HUB]', error?.message);
    return NextResponse.json({ items: [], packs: [], totalCount: 0 });
  }
}
