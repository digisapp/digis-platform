...
export async function GET() {
  try {
    ...
    const messages = await db.query.cloudLockedMessages.findMany({
      where: eq(cloudLockedMessages.creatorId, user.id),
      orderBy: [desc(cloudLockedMessages.sentAt)],
      with: {
    ...
    console.error('[CLOUD LOCKED MESSAGES GET]', { error: error.message });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [lockedMessage] = await db.insert(cloudLockedMessages).values({
      creatorId: user.id,
    ...
    // Attach items
    await db.insert(cloudLockedMessageItems).values(
    ...
    await db.insert(cloudLockedMessageRecipients).values(
    ...
    const notifPromises = recipients.map(recipientId =>
      NotificationService.sendNotification(
        ...
        { creatorId: user.id, lockedMessageId: lockedMessage.id, type: 'cloud_locked_message' }
    ...
    console.error('[CLOUD LOCKED MESSAGES POST]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
