...
import { db, cloudPacks, cloudPackItems, cloudItems } from '@/lib/data/system';
...
export async function POST(
  ...
  try {
    ...
    const items = await db.select()
      .from(cloudItems)
      .where(and(eq(cloudItems.creatorId, user.id)));
    ...
    const existing = await db.select()
      .from(cloudPackItems)
      .where(eq(cloudPackItems.packId, id));
    ...
    await db.update(cloudPacks)
      .set({
        itemCount: sql`${cloudPacks.itemCount} + ${newIds.length}`,
        updatedAt: new Date(),
      })
    ...
    console.error('[CLOUD PACK ITEMS POST]', { error: error.message });
...
export async function DELETE(
  ...
  try {
    ...
    for (const itemId of itemIds) {
      const result = await db.delete(cloudPackItems)
        .where(and(
          eq(cloudPackItems.packId, id),
          eq(cloudPackItems.itemId, itemId),
        ));
      removed++;
    }
    ...
    console.error('[CLOUD PACK ITEMS DELETE]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
