...
import { db, cloudItems, cloudPurchases, wallets, walletTransactions } from '@/lib/data/system';
...
/**
 * POST - Purchase a single Drops item
 */
export async function POST(
  ...
  try {
    ...
    const item = await db.query.cloudItems.findFirst({
      where: and(eq(cloudItems.id, itemId), eq(cloudItems.status, 'live')),
    });
    ...
    const existing = await db.query.cloudPurchases.findFirst({
      where: and(
        ...
        eq(cloudPurchases.itemId, itemId),
      ),
    });
    ...
    const idempotencyKey = `cloud_item_${user.id}_${itemId}_${nanoid(8)}`;
    ...
    const [buyerTx] = await db.insert(walletTransactions).values({
      ...
      type: 'cloud_purchase',
      status: 'completed',
      description: `Cloud item purchase`,
      idempotencyKey,
    ...
    console.error('[CLOUD PURCHASE ITEM]', { error: error.message });
