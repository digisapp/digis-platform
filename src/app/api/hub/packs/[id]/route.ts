...
import { db, hubPacks, hubPackItems } from '@/lib/data/system';
  ...
  try {
    ...
    const pack = await db.query.hubPacks.findFirst({
      ...
      with: {
        items: {
          with: { item: true },
          orderBy: (hubPackItems, { asc }) => [asc(hubPackItems.sortOrder)],
        },
      },
    });
    ...
    console.error('[HUB PACK GET]', { error: error.message });
...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db.update(hubPacks)
      .set(updates)
    ...
    console.error('[HUB PACK PATCH]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await db.delete(hubPacks).where(eq(hubPacks.id, id));

    return NextResponse.json({ deleted: true });
  } catch (error: any) {
    console.error('[HUB PACK DELETE]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
