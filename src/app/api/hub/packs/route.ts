...
import { db, users, hubPacks, hubPackItems, hubItems } from '@/lib/data/system';
...
export async function GET() {
  try {
    ...
    const packs = await db.query.hubPacks.findMany({
      where: eq(hubPacks.creatorId, user.id),
    ...
    console.error('[HUB PACKS GET]', { error: error.message });
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[HUB PACKS POST]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
