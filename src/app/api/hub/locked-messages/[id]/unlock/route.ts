...
import { db, hubLockedMessages, hubLockedMessageRecipients, hubLockedMessageItems, wallets, walletTransactions } from '@/lib/data/system';
...
export async function POST(
  ...
  try {
    ...
    const recipient = await db.query.hubLockedMessageRecipients.findFirst({
      where: and(
        eq(hubLockedMessageRecipients.messageId, messageId),
        eq(hubLockedMessageRecipients.recipientId, user.id),
      ),
    });
    ...
    // Get message details
    const message = await db.query.hubLockedMessages.findFirst({
      where: eq(hubLockedMessages.id, messageId),
    ...
    const idempotencyKey = `hub_lm_${user.id}_${messageId}_${nanoid(8)}`;

    // Debit buyer
    const [buyerTx] = await db.insert(walletTransactions).values({
      userId: user.id,
      amount: -price,
      type: 'hub_locked_message',
      status: 'completed',
    ...
    await db.insert(walletTransactions).values({
      ...
      amount: price,
      type: 'hub_locked_message_earnings',
      status: 'completed',
    ...
    // Mark as unlocked
    await db.update(hubLockedMessageRecipients)
      .set({
        unlocked: true,
        unlockedAt: new Date(),
        transactionId: buyerTx.id,
      })
      .where(eq(hubLockedMessageRecipients.id, recipient.id));

    // Update message stats
    await db.update(hubLockedMessages)
      .set({
        unlockCount: sql`${hubLockedMessages.unlockCount} + 1`,
        totalEarnings: sql`${hubLockedMessages.totalEarnings} + ${price}`,
      })
      .where(eq(hubLockedMessages.id, messageId));
    ...
    console.error('[HUB LOCKED MESSAGE UNLOCK]', { error: error.message });
