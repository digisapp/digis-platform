...
import { db, users, hubItems, hubScheduledDrops, creatorPricingDefaults } from '@/lib/data/system';
...
export async function GET() {
  try {
    ...
    const drops = await db.query.hubScheduledDrops.findMany({
      where: eq(hubScheduledDrops.creatorId, user.id),
      orderBy: [desc(hubScheduledDrops.scheduledFor)],
      with: { item: true },
    });

    return NextResponse.json({ drops });
  } catch (error: any) {
    console.error('[HUB DROPS GET]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}

/**
 * POST - Schedule drops for Hub items
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (defaults) {
      for (const item of items) {
        if (!item.priceCoins) {
          ...
          if (price) {
            await db.update(hubItems).set({ priceCoins: price }).where(eq(hubItems.id, item.id));
          }
        }
      }
    }
    ...
    // Insert drops
    await db.insert(hubScheduledDrops).values(dropRecords);
    ...
    console.error('[HUB DROPS POST]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
  ...
  try {
    ...
    if (batchId) {
      await db.update(hubScheduledDrops)
        ...
        .where(and(
          eq(hubScheduledDrops.creatorId, user.id),
          eq(hubScheduledDrops.batchId, batchId),
      ...
      await db.update(hubScheduledDrops)
        ...
        .where(and(
          eq(hubScheduledDrops.creatorId, user.id),
          inArray(hubScheduledDrops.id, dropIds),
          eq(hubScheduledDrops.status, 'scheduled'),
        ));
    } else {
    ...
    console.error('[HUB DROPS DELETE]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
