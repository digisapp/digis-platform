...
import { db, hubItems, creatorPricingDefaults } from '@/lib/data/system';
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (action === 'price_all') {
      ...
      const unpricedItems = await db.select()
        ...
        .where(and(
          ...
          eq(hubItems.priceCoins, 0), // Will also catch null via IS NULL below
        ));
    ...
    console.error('[HUB ITEMS BULK]', { error: error.message });
