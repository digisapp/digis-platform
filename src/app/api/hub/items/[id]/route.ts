...
import { db, hubItems, users } from '@/lib/data/system';
...
export async function GET(
  ...
  try {
    ...
    const item = await db.query.hubItems.findFirst({
      where: and(eq(hubItems.id, id), eq(hubItems.creatorId, user.id)),
    });
    ...
    console.error('[HUB ITEM GET]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}

/**
 * PATCH - Update Hub item (price, status)
 */
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db.update(hubItems)
      .set(updates)
    ...
    console.error('[HUB ITEM PATCH]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}

/**
 * DELETE - Remove Hub item and free storage
 */
export async function DELETE(
  ...
  try {
    ...
    const bucket = 'hub-content';
    // Extract path from public URL
    const pathMatch = fileUrl.match(/hub-content\/(.+)$/);
    if (pathMatch) {
    ...
    console.error('[HUB ITEM DELETE]', { error: error.message });
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 });
  }
}
