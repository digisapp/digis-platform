...
import { db, hubItems } from '@/lib/data/system';
 ...
 * Generates thumbnail + preview for a hub item
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!thumbErr) {
      thumbnailUrl = supabase.storage.from('hub-content').getPublicUrl(thumbPath).data.publicUrl;
    }
    ...
    if (!prevErr) {
      previewUrl = supabase.storage.from('hub-content').getPublicUrl(previewPath).data.publicUrl;
    }
