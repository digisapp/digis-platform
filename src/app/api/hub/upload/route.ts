...
const MAX_VIDEO_SIZE = 500 * 1024 * 1024; // 500MB (larger for Hub — creators upload full camera roll)
const HUB_STORAGE_QUOTA = 50 * 1024 * 1024 * 1024; // 50GB

/**
 * POST - Upload one or more files to Hub
...
export async function POST(request: NextRequest) {
  try {
    ...
    const createdItems: Array<typeof hubItems.$inferSelect> = [];
    ...
    try {
      for (const { file, type, sizeBytes } of fileInfos) {
        ...
        if (uploadError) {
          console.error('[HUB UPLOAD] Storage error:', { error: uploadError, fileName });
      ...
      console.error('[HUB UPLOAD] Error:', { error: error.message, userId: user.id });
    ...
    console.error('[HUB UPLOAD ERROR]', { error: error.message });
