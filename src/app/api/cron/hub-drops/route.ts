...
import { hubScheduledDrops, hubItems, hubCreatorStreaks } from '@/db/schema';
...
/**
 * GET - Process scheduled Hub drops (called by Vercel Cron)
...
export async function GET(req: NextRequest) {
  try {
    ...
    const dueDrops = await db.select()
      .from(hubScheduledDrops)
      .where(and(
        eq(hubScheduledDrops.status, 'scheduled'),
        lte(hubScheduledDrops.scheduledFor, now),
      ));
    ...
    for (const drop of dueDrops) {
      try {
        ...
        const item = await db.query.hubItems.findFirst({
          where: eq(hubItems.id, drop.itemId),
        });

        if (!item) {
          // Item was deleted, cancel the drop
          await db.update(hubScheduledDrops)
            .set({ status: 'cancelled' })
            .where(eq(hubScheduledDrops.id, drop.id));
          continue;
        }
        ...
        NotificationService.notifyFollowersOfDrop(drop.creatorId, item).catch(err => {
          console.error('[CRON HUB DROPS] Notification error:', err.message);
        });

      } catch (err: any) {
        console.error('[CRON HUB DROPS] Error publishing drop:', { dropId: drop.id, error: err.message });
        errors++;
      }
    }
    ...
    for (const creatorId of creatorsUpdated) {
      try {
        const existing = await db.query.hubCreatorStreaks.findFirst({
          where: eq(hubCreatorStreaks.creatorId, creatorId),
        });
        ...
        if (existing) {
          ...
          if (!lastActive || lastActive.getTime() !== today.getTime()) {
            await db.update(hubCreatorStreaks)
              .set({
          ...
          await db.insert(hubCreatorStreaks).values({
            creatorId,
            currentStreak: 1,
            longestStreak: 1,
            lastActiveDate: now,
        ...
        console.error('[CRON HUB DROPS] Streak update error:', { creatorId, error: err.message });
      }
    }
    ...
    console.error('[CRON HUB DROPS ERROR]', { error: error.message });
