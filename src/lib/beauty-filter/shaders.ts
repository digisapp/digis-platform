...
export const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // Bilateral filter for skin smoothing (25-tap, 5x5 kernel)
    ...
    for (int x = -2; x <= 2; x++) {
      for (int y = -2; y <= 2; y++) {
        ...
        vec4 sam = texture2D(u_image, v_texCoord + offset);

        // Spatial weight (Gaussian, sigma ~1.5)
        float spatialDist = length(vec2(float(x), float(y)));
        float spatialWeight = exp(-spatialDist * spatialDist / 4.5);

        // Intensity weight (preserves edges, softer threshold)
        float intensityDiff = length(sam.rgb - center.rgb);
        float intensityWeight = exp(-intensityDiff * intensityDiff * 25.0);
    ...
    // Soft glow: screen blend of wider blur with result
    vec4 blurred = vec4(0.0);
    for (int x = -2; x <= 2; x++) {
      for (int y = -2; y <= 2; y++) {
        vec2 offset = vec2(float(x), float(y)) * texelSize * 6.0;
