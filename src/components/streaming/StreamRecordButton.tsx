  ...
  if (isRecording) {
    return (
      <button
        ...
        className={`flex items-center backdrop-blur-xl bg-red-500/30 rounded-full border border-red-500 text-white font-semibold hover:bg-red-500/40 transition-all animate-pulse ${
          compact ? 'gap-1.5 px-2.5 py-1.5' : 'gap-2 px-4 py-2.5'
        }`}
      >
        <div className="relative">
          <Circle className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} text-red-500 fill-red-500`} />
          <div className="absolute inset-0 animate-ping">
            <Circle className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} text-red-500 fill-red-500 opacity-50`} />
          </div>
        </div>
        <span className={`text-red-400 font-bold tabular-nums ${compact ? 'text-xs' : 'text-sm'}`}>{currentDuration}</span>
        <Square className={`${compact ? 'w-2.5 h-2.5' : 'w-3 h-3'} text-white fill-white`} />
      </button>
    );
  }
  ...
  if (compact) {
    return (
      <button
        ...
        disabled={!canRecord}
        className={`flex items-center justify-center p-2 backdrop-blur-xl rounded-full border transition-all ${
          canRecord
