...
import type { HubItem } from '@/hooks/useHubData';

interface HubGridProps {
  items: HubItem[];
  selectedItems: Set<string>;
  onToggleSelect: (id: string) => void;
  onItemClick: (item: HubItem) => void;
...
const HubGridItem = memo(({ item, selected, selectionMode, onToggleSelect, onItemClick }: {
  item: HubItem;
  selected: boolean;
  selectionMode: boolean;
  onToggleSelect: (id: string) => void;
...
HubGridItem.displayName = 'HubGridItem';

export function HubGrid({ items, selectedItems, onToggleSelect, onItemClick, selectionMode }: HubGridProps) {
  if (items.length === 0) {
    return (
